...
answers to the design objections, quoted verbatim in `.claude/answers.md`), 2026-08-18,
2026-08-24/25 and 2026-08-28. `[owner, improvement N]` — one of the **three** improvements ordered
after PRD v2.3 and quoted verbatim in PRD §7.5; the first two carried no date and none is invented,
the third carries 2026-08-28 and it is used.
...
| **v2.5** | The third improvement (PRD §7.5): an opened panel is brought into view, always and by the minimum offset, the 50 % threshold dropped under the owner's «if it is possible». **AC 30 added**, in six clauses that fail separately because a human renders the verdict. The §7.2 premise recomputed on the shipped data (`Auswe` 14 places split 9 + 5, `Brmn` 10 flat) and the accepted cost restated in Russian, the binding locale; §5 records the measured band of the descriptions — 36 texts, 88–304 characters — as a measurement and not a limit. Threads D11, D16 and D17 answered. |
| **v2.6** | Two corrections and no rule. **AC 30 clause c gets a way of creating the state it judges** (thread A1, opened by the architect): the shipped texts do not make a panel taller than a phone window at the default text size, so the clause was unfailable as written — it now names enlarged browser text as the condition, the longest description in the set as the subject, and "the case was not created" as a verdict distinct from "passed". §7.5's prose carried the same false premise and is corrected with it. **The missing threshold becomes the owner's own decision** (D17, closed 2026-08-28), with his reason for having named 50 % — it guarded **noticeability**, not readability, which is not what v2.5 recorded; the conclusion is unchanged and clause b is untouched. |
| **v2.7** | The product half of thread D18: design made the control mark conditional on scripts being available (v1.4 §3.2) and the architect built it (v1.3 §4.3.2), while §10's requirement — a visitor tells a clickable name from an inert one before clicking — had no criterion. **AC 31 added**: the mark is carried only where the click works, checked in one action by switching scripts off and back. Two accepted states are named inside it so a checker files nothing on them — the server-rendered button role that survives without scripts, and the pre-hydration window, which is deliberately left unjudged because its outcome is not promised. Print is scoped out of the criterion — judged on screen — and left there by instruction, one move behind the thread: design settled it the same day (no print rule, the mark prints, and a printed page has no click for it to promise), so the addition is one line inside AC 31 rather than a criterion. |
